...
auth.post("/signup", async (c) => {
  try {
    ...
    await c.env.DB.prepare("INSERT INTO users (id, email, password_hash) VALUES (?, ?, ?)").bind(userId, email, passHash).run();
...
auth.post("/login", async (c) => {
  const { email, password } = await c.req.json();
  const user = await c.env.DB.prepare("SELECT id, password_hash FROM users WHERE email = ?").bind(email).first();
  ...
  await c.env.DB.prepare("INSERT INTO sessions (id, user_id, expires_at) VALUES (?, ?, ?)").bind(token, user.id, expiresAt).run();
...
merchant.post("/profile/ensure", async (c) => {
  ...
  const merchantId = Math.floor(1e4 + Math.random() * 9e4).toString();
  try {
...
// .wrangler/tmp/bundle-Hv6xzt/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Hv6xzt/middleware-loader.entry.ts
