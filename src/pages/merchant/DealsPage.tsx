...
import { agreementTemplates, merchantAgreements } from '@/lib/api';
...
function canDeleteAgreement(agreement: MerchantAgreement, usedAgreementIds: Set<string>) {
  return (agreement.status === 'pending' || agreement.status === 'rejected') && !usedAgreementIds.has(agreement.id);
}
...
export default function DealsPage() {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    const [templatesRes, agreementsRes] = await Promise.allSettled([
  ...
  const confirmDelete = async () => {
    ...
    if (!agreement) return;
    if (usedAgreementIds.has(agreement.id)) {
      ...
      toast.success('Used merchant agreement archived to protect order history.');
    } else {
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {agreements.map(agreement => {
                  ...
                  const deletable = canDeleteAgreement(agreement, usedAgreementIds);
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          {deletable ? (
                            ...
                            <button onClick={() => setDeleteDealId(agreement.id)} className="px-3 py-1.5 text-xs font-medium rounded-md border border-warning/30 bg-warning/5 text-warning hover:bg-warning/10 transition-colors">Archive</button>
                          )}
                        </div>
                      </td>
                    </tr>
      ...
      <Dialog open={!!deleteDealId} onOpenChange={open => !open && setDeleteDealId(null)}>
        <DialogContent className="sm:max-w-[440px]">
          <DialogHeader>
            ...
            <DialogDescription>Delete stays easy in the current UI. The backend archives used agreements instead of breaking order history.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
