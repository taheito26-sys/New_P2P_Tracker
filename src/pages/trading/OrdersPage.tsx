...
function OrdersPageWorkspace() {
  ...
  const selectedAgreement = useMemo(
    () => approvedAgreements.find((agreement) => agreement.id === selectedAgreementId) || null,
    [approvedAgreements, selectedAgreementId],
  );
        ...
        <div>
          <div className="panel" style={{ padding: 12 }}>
            ...
            <div className="grid2" style={{ display: 'grid', gap: 8, gridTemplateColumns: 'repeat(2, minmax(0,1fr))' }}>
              ...
              <label className="field"><span>{t('agreementTypeLabel')}</span><select value={selectedAgreementId} onChange={(e) => setSelectedAgreementId(e.target.value)}><option value="">Approved agreements only</option>{approvedAgreements.map((agreement) => <option key={agreement.id} value={agreement.id}>{agreement.title}</option>)}</select></label>
